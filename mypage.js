    ...
    if (user) {
        if (infoArea) {
            ...
            infoArea.innerHTML = `
                ...
                }" alt="프로필" style="width:56px;height:56px;border-radius:50%;background:#ececec;object-fit:cover;margin-bottom:8px;" />
                <div style="font-size:1.1rem;font-weight:600;color:#72d1ff;margin-top:6px;">${
                    user.displayName || user.email
                }</div>
                <div style="font-size:0.98rem;color:#ececec;margin-top:2px;">${
                    user.email
                }</div>
            `;
        }
        box.innerHTML = `<span class="user-email" style="text-decoration:underline;text-underline-position:under;color:#72d1ff;cursor:pointer;gap:6px;display:flex;align-items:center;">
            <img src="${
                user.photoURL || "/image/hssh_Logo.png"
            }" alt="프로필" style="width:28px;height:28px;border-radius:50%;background:#ececec;object-fit:cover;margin-right:4px;" />
            ${user.email}
        </span>
...
async function fetchAndShowWeeklyTimetable(grade, classNum, timetableBox) {
    if (!neisTimetableApiKey) {
        timetableBox.innerHTML =
            '<div style="text-align:center;color:#ff4d4f;margin-top:18px;">시간표 API 키가 설정되지 않았습니다.</div>';
    ...
    timetableBox.innerHTML =
        '<div style="text-align:center;">전체 시간표 불러오는 중...</div>';
    try {
        ...
        if (rows.length === 0) {
            timetableBox.innerHTML =
                '<div style="text-align:center;margin-top:18px;">이번 주 시간표가 없습니다.</div>';
            return;
        }
        ...
        let html =
            '<div class="timetable-wide-card"><div style="font-weight:600;font-size:1.08rem;margin-bottom:8px;text-align:center;color:#72d1ff;">이번 주 전체 시간표</div>';
        html +=
            '<div style="overflow-x:auto;"><table class="weekly-timetable">';
        ...
        timetableBox.innerHTML =
            '<div style="text-align:center;color:#ff4d4f;margin-top:18px;">주간 시간표 불러오기 실패</div>';
    }
}
